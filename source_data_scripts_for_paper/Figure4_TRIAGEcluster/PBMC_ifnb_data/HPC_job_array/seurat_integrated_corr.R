...
for (i in clustnum){
    ...
    immune.combined.sct_1_df <- as.data.frame(as.matrix(GetAssayData(immune.combined.sct_1, assay = "integrated", slot = "data")))
    corr <- as.data.frame(cor(immune.combined.sct_1_df, method = "spearman"))   
...
saveRDS(dfls.all, paste0("PBMC_ifnb_data/Seurat_integrated_correlation/res_", seures[n], "_integrated_withincluster_mean_correlation.RDS"))
